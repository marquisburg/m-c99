#include "lexer.h"
#include "lower.h"
#include "parser.h"
#include "sema.h"

#include <mtlc/build.h>
#include <mtlc/mtlc.h>

#include <stdio.h>
#include <string.h>

static void usage(const char *argv0) {
  fprintf(stderr,
          "C99Mettle — C99 compiler (libmtlc backend)\n"
          "Usage: %s [options] <file.c>\n"
          "Options:\n"
          "  -o <path>     output executable (default: a.exe / a.out)\n"
          "  -O0/-O1/-O    optimization off / on\n"
          "  -c            emit object only (.o / .obj)\n"
          "  -S            emit object (same as -c; no asm text yet)\n"
          "  --emit-ir     finish after lowering (smoke test; no output file)\n"
          "  -h, --help    this help\n"
          "\nBackend: %s\n",
          argv0, mtlc_version());
}

int main(int argc, char **argv) {
  const char *input = NULL;
  const char *output = NULL;
  int opt_level = 1;
  int obj_only = 0;
  int emit_ir_only = 0;

  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
      usage(argv[0]);
      return 0;
    }
    if (strcmp(argv[i], "-o") == 0) {
      if (i + 1 >= argc)
        fatal("missing argument for -o");
      output = argv[++i];
      continue;
    }
    if (strcmp(argv[i], "-O0") == 0) {
      opt_level = 0;
      continue;
    }
    if (strcmp(argv[i], "-O") == 0 || strcmp(argv[i], "-O1") == 0 ||
        strcmp(argv[i], "-O2") == 0 || strcmp(argv[i], "-O3") == 0) {
      opt_level = 1;
      continue;
    }
    if (strcmp(argv[i], "-c") == 0 || strcmp(argv[i], "-S") == 0) {
      obj_only = 1;
      continue;
    }
    if (strcmp(argv[i], "--emit-ir") == 0) {
      emit_ir_only = 1;
      continue;
    }
    if (argv[i][0] == '-')
      fatal("unknown option '%s'", argv[i]);
    if (input)
      fatal("multiple input files not supported");
    input = argv[i];
  }

  if (!input) {
    usage(argv[0]);
    return 1;
  }

  if (!output) {
#ifdef _WIN32
    output = obj_only ? "a.obj" : "a.exe";
#else
    output = obj_only ? "a.o" : "a.out";
#endif
  }

  size_t src_len = 0;
  char *src = read_file(input, &src_len);
  if (!src)
    fatal("cannot read '%s'", input);

  Arena arena;
  arena_init(&arena);
  Diag diag = {0};
  TypeContext tc;
  type_context_init(&tc, &arena);

  Lexer lexer;
  lexer_init(&lexer, &arena, &diag, input, src, src_len);

  Parser parser;
  parser_init(&parser, &lexer, &tc);
  Program *prog = parse_program(&parser);
  if (diag.error_count) {
    fprintf(stderr, "%d error(s) generated.\n", diag.error_count);
    free(src);
    arena_free(&arena);
    return 1;
  }

  Sema sema;
  sema_init(&sema, &arena, &diag, &tc);
  sema_check(&sema, prog);
  if (diag.error_count) {
    fprintf(stderr, "%d error(s) generated.\n", diag.error_count);
    free(src);
    arena_free(&arena);
    return 1;
  }

  MtlcModule *mod = lower_program(&sema, prog, &diag);
  if (!mod || diag.error_count) {
    fprintf(stderr, "lowering failed (%d error(s)).\n", diag.error_count);
    free(src);
    arena_free(&arena);
    return 1;
  }

  if (emit_ir_only) {
    printf("OK: lowered %s (%zu functions)\n", input,
           mtlc_module_function_count(mod));
    mtlc_module_destroy(mod);
    free(src);
    arena_free(&arena);
    return 0;
  }

  MtlcContext *ctx = mtlc_context_create();
  if (!ctx)
    fatal("mtlc_context_create failed");
  mtlc_context_set_opt_level(ctx, opt_level);
  mtlc_context_set_whole_program(ctx, obj_only ? 0 : 1);

  if (opt_level > 0) {
    if (!mtlc_optimize(ctx, mod)) {
      fprintf(stderr, "optimization failed\n");
      mtlc_context_destroy(ctx);
      mtlc_module_destroy(mod);
      free(src);
      arena_free(&arena);
      return 1;
    }
  }

  int ok;
  if (obj_only)
    ok = mtlc_emit_object(ctx, mod, output);
  else
    ok = mtlc_build_executable(ctx, mod, output);

  mtlc_context_destroy(ctx);
  mtlc_module_destroy(mod);
  free(src);
  arena_free(&arena);

  if (!ok) {
    fprintf(stderr, "code generation / link failed\n");
    return 1;
  }
  return 0;
}
